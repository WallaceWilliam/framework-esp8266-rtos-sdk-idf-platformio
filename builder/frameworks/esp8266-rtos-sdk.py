...
def generate_project_ld_script(sdk_config, ignore_targets=None):
    ...
    return env.Command(
        join("$BUILD_DIR", "esp8266.project.ld"),
        join(FRAMEWORK_DIR, "components", "esp8266", "ld", "esp8266.project.ld.in"),
...
def build_bootloader():
    ...
    code_model = get_cmake_code_model(
        ...
        [
            ...
            "-DIDF_PATH=" + env.subst("$FRAMEWORK_DIR"), #platform.get_package_dir("framework-espidf"),
            "-DSDKCONFIG=" + to_unix_path(join(env.subst("$PROJECT_DIR"), "sdkconfig")),
